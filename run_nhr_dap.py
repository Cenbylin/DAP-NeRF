...
def config_parser():
    ...
    parser.add_argument("--fre_test", type=int, default=99999999,
                        help='frequency of test')
    return parser

@torch.no_grad()
def render_viewpoints_nhr(model, data_class, ndc, render_kwargs, test=True, 
                          savedir=None, eval_psnr=False, eval_lpips_alex=False, eval_lpips_vgg=False):
    ...
    for cnt, i in tqdm(enumerate(idx)):
        ret = data_class.load_idx(i)
        rays_o, rays_d = ret['rays_ori'], ret['rays_dir']
        viewdirs, rgb_gt = ret['viewdirs'], ret['rays_color']
        ...
        render_result_chunks = [
            ...
            for ro, rd, vd ,ts in zip(rays_o.split(bacth_size, 0), rays_d.split(bacth_size, 0),
        ...
        if eval_psnr:
            ...
            psnrs.append(p)
            rgbs_list.append(torch.from_numpy(np.clip(rgb,0,1)).reshape(-1,data_class.h,data_class.w))
            rgbs_gt_list.append(torch.from_numpy(np.clip(rgb_gt,0,1)).reshape(-1,data_class.h,data_class.w))
        if eval_lpips_alex:
        ...
        if eval_psnr:
            rgbs_tensor = torch.stack(rgbs_list[-1:], 0)
            rgbs_gt_tensor = torch.stack(rgbs_gt_list[-1:], 0)
            ms_ssims.append(ms_ssim(rgbs_gt_tensor, rgbs_tensor, data_range=1, size_average=True).item())
        ...
        if savedir is not None:
            rgb8 = utils.to8b(rgb)
            filename = os.path.join(savedir, '{:03d}.png'.format(cnt))
    ...
    if savedir is not None:
        ...
        if len(psnrs):
            ...
            with open(os.path.join(savedir, 'ms_ssim.txt'), 'w') as f:
                f.write('ms_ssim: {:.4f}'.format(np.mean(ms_ssims)))
            if eval_lpips_alex:
...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        H, W = HW[i]
        K = Ks[i]
        rays_o, rays_d, viewdirs = dapnerf.get_rays_of_a_view(
                H, W, K, c2w, ndc)
        keys = ['rgb_marched', 'depth']
...
def load_everything(args, cfg):
    ...
    if cfg.data.dataset_type in ['hyper_dataset', 'nhr']:
        kept_keys = {
            'data_class',
            'near', 'far',
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict):
    ...
    if cfg.data.dataset_type not in ['hyper_dataset', 'nhr']:
        ...
        # [added]
        times_i_train = torch.tensor(data_class.all_time, dtype=torch.float32).to(device)[i_train]
        times_i_train = torch.sort(times_i_train.unique())[0]
    ...
    def gather_training_rays_hyper():
        ...
        for i in data_class.i_train:
            ...
            top += n
        assert top == N
        same_im = True  # [added] mainly to sync time
        if same_im:
            index_generator = dapnerf.batch_indices_generator_sameim(
                imsz, cfg_train.N_rand)
        else:
            index_generator = dapnerf.batch_indices_generator(
                len(rgb_tr), cfg_train.N_rand)
    ...
    def gather_training_rays():
        ...
        if cfg_train.ray_sampler == 'in_maskcache':
            ...
            rgb_tr, times_flaten,rays_o_tr, rays_d_tr, viewdirs_tr, imsz = dapnerf.get_training_rays_in_maskcache_sampling(
            ...
            rgb_tr, times_flaten,rays_o_tr, rays_d_tr, viewdirs_tr, imsz = dapnerf.get_training_rays_flatten(
                rgb_tr_ori=rgb_tr_ori,times=times_i_train,
                train_poses=poses[i_train],
                HW=HW[i_train], Ks=Ks[i_train], ndc=cfg.data.ndc,)
        else:
            print('cfg_train.ray_sampler =random')
            rgb_tr, times_flaten,rays_o_tr, rays_d_tr, viewdirs_tr, imsz = dapnerf.get_training_rays(
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ...
        if global_step in cfg_train.pg_scale:
            ...
            if isinstance(model, dapnerf.TiNeuVox):
                model.scale_volume_grid(cur_voxels)
            else:
                raise NotImplementedError
            optimizer = utils.create_optimizer_or_freeze_model(model, cfg_train, global_step=0)

        # [added] remove redundant particles
        if global_step in cfg_train.remove_particle_steps \
            and model.enabled_particle:
            if global_step != cfg_train.remove_particle_steps[-1]:
                alpha_thres = cfg_train.remove_particle_alpha_thres[0]
            else:
                alpha_thres = cfg_train.remove_particle_alpha_thres[1]

            model.remove_redundant_particle(
                times_i_train, cfg_train.remove_particle_traj_thres, 
                render_kwargs["stepsize"]*model.voxel_size, alpha_thres,
                cfg_train.strict_traj)
            model.reample_particles(cfg_train.resample_target)

            optimizer = utils.create_optimizer_or_freeze_model(
                model, cfg_train, global_step=global_step, print_info=False)
            torch.cuda.empty_cache()

        # random sample rays
        if cfg.data.dataset_type == 'nhr':
            im_id = np.random.randint(0, len(data_class.i_train))
            ret = data_class.load_idx(im_id)
            rays_o_, rays_d_ = ret['rays_ori'], ret['rays_dir']
            viewdirs_, rgb_ = ret['viewdirs'], ret['rays_color']
            sel_i = np.random.randint(0, len(rays_o_), cfg_train.N_rand)

            rays_o = rays_o_[sel_i].to(device)
            rays_d = rays_d_[sel_i].to(device)
            viewdirs = viewdirs_[sel_i].to(device)
            target = rgb_[sel_i].to(device)
            times_sel = (torch.zeros(len(rays_o_), 1) + data_class.all_time[im_id]).to(device)

        elif cfg_train.ray_sampler in ['flatten', 'in_maskcache'] or cfg.data.dataset_type == 'hyper_dataset':
        ...
        if model.use_occ_grid:
            model.occ_grid.update_every_n_steps(
                step=global_step - 1,
        ...
        # use learned occ mask to sample particles
        if global_step > cfg_train.init_particle_after and (not model.enabled_particle):
            bg_points = None
            if cfg.data.dataset_type == 'hyper_dataset':
                if data_class.use_bg_points:
                    bg_points = data_class.bg_points
            model.init_particles(bg_points)
        ...
        if global_step<cfg_train.tv_before and global_step>cfg_train.tv_after and global_step%cfg_train.tv_every==0:
            if cfg_train.weight_tv_motion>0:
                tv_loss_motion = model.motion_tv_loss()  # current
                loss += cfg_train.weight_tv_motion * tv_loss_motion
            if cfg_train.weight_tv_feature>0:
                tv_loss_feat = model.fuse_feature_tv_loss()
                loss += cfg_train.weight_tv_feature * tv_loss_feat
        ...
        if global_step%args.i_print == 0:
            ...
            tqdm.write(f'scene_rep_reconstruction : iter {global_step:6d} / '
                       f'particle_N: {model.particle_num if model.enabled_particle else 0} / '
                       f'tv_motion: {tv_loss_motion.item() if "tv_loss_motion" in locals() else 0:.4f} / '
                       f'tv_feat: {tv_loss_feat.item() if "tv_loss_feat" in locals() else 0:.4f} / '
                       f'PSNR: {np.mean(psnr_lst):5.2f} / Eps: {eps_time_str}')
            psnr_lst = []
...
def train(args, cfg, data_dict=None):
    ...
    elif cfg.data.dataset_type == "nhr":
        if data_dict['data_class'].aabb is None:
            raise ValueError('need data_dict.aabb')
        xyz_min, xyz_max = torch.tensor(data_dict['data_class'].aabb).reshape(2, 3)
    else:
        if data_dict is not None and "aabb" in cfg.data:
            xyz_min, xyz_max = torch.tensor(cfg.data["aabb"]).reshape(2, 3)
        else:
...
if __name__=='__main__':
    ...
    if args.render_train:
        ...
        if cfg.data.dataset_type  not in ['hyper_dataset', 'nhr']:
            ...
            rgbs, disps = render_viewpoints_nhr(
                    data_calss=data_dict['data_calss'],
                    savedir=testsavedir, all=True, test=False,
                    eval_psnr=args.eval_psnr,
                    eval_lpips_alex=args.eval_lpips_alex, eval_lpips_vgg=args.eval_lpips_vgg,
